...
public sealed class PropertyVisit : Entity
{
    ...
    private PropertyVisit(Guid propertyId,Guid healthUnitId,Guid agentId,DateTimeOffset visitedAtUtc,VisitType type,VisitOutcome outcome,PropertySituation observedSituation,bool accessDifficulty,string? note)
    {PropertyId=propertyId;HealthUnitId=healthUnitId;AgentId=agentId;VisitedAtUtc=visitedAtUtc;Type=type;Outcome=outcome;ObservedSituation=observedSituation;AccessDifficulty=accessDifficulty;Note=Clean(note);}
    ...
    public static PropertyVisit Create(Guid propertyId,Guid healthUnitId,Guid agentId,DateTimeOffset date,VisitType type,VisitOutcome outcome,PropertySituation situation,bool difficulty,string? note)=>new(propertyId,healthUnitId,agentId,date,type,outcome,situation,difficulty,note);
