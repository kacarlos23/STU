...
export function OperationalWorkspace({ session, global = false }: { session: Session; global?: boolean }) {
  ...
  async function action(job: Job, kind: 'approve' | 'retry') {
    if (kind === 'approve' && !await confirm({
      ...
      message: `Até agora nenhum dado foi efetivado. Ao continuar, ${job.recordCount.toLocaleString('pt-BR')} imóveis validados serão gravados na UBS e a decisão ficará registrada na auditoria.`,
