...
export function OperationalWorkspace({ session, global = false }: { session: Session; global?: boolean }) {
  ...
  return <section className="operations-workspace">
    ...
    <div className="operations-grid">
      ...
      {canImport && <form className="operations-card" onSubmit={createImport}><span className="operations-kicker">Entrada controlada</span><h3>Importar imóveis</h3><p>Envie CSV ou GeoJSON. O sistema apenas efetiva os registros depois da sua aprovação.</p><label className="operations-file">Arquivo<input accept=".csv,.json,.geojson,text/csv,application/geo+json" name="file" onChange={event => setSelectedFile(event.target.files?.[0] ?? null)} required type="file" /><small>{selectedFile ? `${selectedFile.name} · ${formatBytes(selectedFile.size)}` : 'Máximo de 20 MB e 10.000 imóveis.'}</small></label><details><summary>Colunas esperadas no CSV</summary><code>microregionCode,street,houseNumber,familyNumber,familyResponsibleName,longitude,latitude,situation,registrationStatus</code></details><button className="operations-primary" disabled={busy || !unitId || !selectedFile} type="submit">{selectedFile ? `Validar ${selectedFile.name}` : 'Selecione um arquivo'}</button></form>}
    </div>
