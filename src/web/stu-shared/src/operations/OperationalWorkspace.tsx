...
import { useUiActions } from '../interaction/InteractionProvider'
import './operations.css'
import './operations-ux.css'
import './operations-import.css'
...
type Job = {
  ...
  originalFileName: string | null; recordCount: number; familyCount: number; linkCount: number; validationErrorCount: number; errorSummary: string | null; attemptCount: number
...
const formatLabels: Record<string, string> = { Csv: 'Planilha CSV', Xlsx: 'Planilha Excel', GeoJson: 'GeoJSON', Kml: 'KML', GeoPackage: 'GeoPackage' }
const propertyImportColumns = [
  ['microregionCode', 'Código da microrregião'], ['street', 'Logradouro'], ['houseNumber', 'Número'],
  ['familyNumber', 'Número da família'], ['familyResponsibleName', 'Responsável pela família'],
  ['postalCode', 'CEP'], ['complement', 'Complemento'], ['longitude', 'Longitude'], ['latitude', 'Latitude'],
  ['registrationStatus', 'Situação cadastral'], ['situation', 'Situação do imóvel'],
]
...
export function OperationalWorkspace({ session, global = false }: { session: Session; global?: boolean }) {
  ...
  const has = (permission: string) => global || session.permissions.includes('*') || session.permissions.includes(permission)
  const canExport = has('reports.export') && has('families.view') && has('properties.view')
  const canImport = has('territory.manage') && has('families.manage') && has('properties.manage')
  ...
  async function createImport(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset(); setSelectedFile(null); setNotice('Arquivo recebido. A validação será feita antes de qualquer cadastro.'); await load()
    } catch (caught) { setError(messageOf(caught)) } finally { setBusy(false) }
  }

  async function action(job: Job, kind: 'approve' | 'retry') {
    if (kind === 'approve' && !await confirm({
      title: `Aprovar ${job.recordCount.toLocaleString('pt-BR')} imóveis?`,
      message: `Até agora nenhum dado foi efetivado. Ao continuar, ${job.recordCount.toLocaleString('pt-BR')} imóveis, ${job.familyCount ?? 0} famílias e ${job.linkCount ?? 0} vínculos serão gravados na UBS e a decisão ficará registrada na auditoria.`,
      confirmLabel: `Aprovar ${job.recordCount.toLocaleString('pt-BR')} imóveis`,
    })) return
  ...
  const awaitingApproval = jobs.find(job => job.kind === 'PropertyImport' && job.status === 'AwaitingApproval')
  const activeImports = jobs.filter(job => job.kind === 'PropertyImport' && (job.status === 'Pending' || job.status === 'Processing')).length
  const draftPercent = indicators && indicators.activeProperties + indicators.drafts > 0 ? Math.round((indicators.drafts / (indicators.activeProperties + indicators.drafts)) * 100) : 0
  ...
  return <section className="operations-workspace">
    ...
    <section className="operations-metrics" aria-label="Indicadores operacionais">
      <OperationMetric detail="cadastros disponíveis na operação" label="Imóveis ativos" value={indicators?.activeProperties} />
      <OperationMetric detail={`${draftPercent}% dos imóveis ativos e rascunhos`} label="Rascunhos" value={indicators?.drafts} />
      <OperationMetric detail="áreas que ainda precisam de responsável" label="Microrregiões sem agente" value={indicators?.unassignedMicroregions} />
      <OperationMetric detail={`${activeImports} importação${activeImports === 1 ? '' : 'ões'} em processamento`} label="Operações pendentes" value={indicators?.pendingJobs} />
    </section>
    {canImport && <section className="operations-flow" aria-label="Etapas da importação"><div className={selectedFile ? 'complete' : 'active'}><i>1</i><span><strong>Selecionar arquivo</strong><small>{selectedFile ? selectedFile.name : 'Excel, CSV ou GeoJSON'}</small></span></div><div className={activeImports ? 'active' : selectedFile ? 'pending' : ''}><i>2</i><span><strong>Validar conteúdo</strong><small>Nenhum dado é gravado</small></span></div><div className={awaitingApproval ? 'active' : ''}><i>3</i><span><strong>Revisar prévia</strong><small>Contagem e erros</small></span></div><div><i>4</i><span><strong>Efetivar</strong><small>Somente após aprovação</small></span></div></section>}
    <div className="operations-grid">
      {canExport && <form className="operations-card" onSubmit={createExport}><span className="operations-kicker">Saída de dados</span><h3>Gerar exportação</h3><p>Filtre os imóveis e escolha o formato compatível com sua ferramenta.</p><label>Formato<select name="format" onChange={event => setExportFormat(event.target.value)} value={exportFormat}><option value="Csv">Planilha CSV</option><option value="GeoJson">GeoJSON</option><option value="Kml">KML</option><option value="GeoPackage">GeoPackage (QGIS)</option></select></label><label>Microrregião<select name="microregionId"><option value="">Todas</option>{microregions.map(item => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}</select></label><label>Situação<select name="situation"><option value="">Todas</option><option value="Active">Ativo</option><option value="Vacant">Vago</option><option value="Abandoned">Abandonado</option><option value="Demolished">Demolido</option></select></label><div className="operations-dates"><label>Visitas desde<input name="from" type="date" /></label><label>Até<input name="to" type="date" /></label></div><button className="operations-primary" disabled={busy || !unitId} type="submit">Gerar {formatLabels[exportFormat] ?? exportFormat}</button></form>}
      {canImport && <form className="operations-card" onSubmit={createImport}><span className="operations-kicker">Entrada controlada</span><h3>Importar imóveis</h3><p>Envie uma planilha Excel (.xlsx), CSV ou GeoJSON. O sistema apenas efetiva os registros depois da sua aprovação.</p><label className="operations-file">Arquivo<input accept=".xlsx,.csv,.json,.geojson,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv,application/geo+json" name="file" onChange={event => setSelectedFile(event.target.files?.[0] ?? null)} required type="file" /><small>{selectedFile ? `${selectedFile.name} · ${formatBytes(selectedFile.size)}` : 'Máximo de 20 MB e 10.000 imóveis.'}</small></label><details className="operations-import-columns"><summary>Campos da planilha</summary><div className="operations-import-columns-table"><table><thead><tr><th>Coluna</th><th>Conteúdo</th></tr></thead><tbody>{propertyImportColumns.map(([name, description]) => <tr key={name}><td><code>{name}</code></td><td>{description}</td></tr>)}</tbody></table></div></details><a className="operations-template-download" download="modelo-importacao-imoveis.xlsx" href="/modelo-importacao-imoveis.xlsx">Baixar planilha Excel modelo</a><button className="operations-primary" disabled={busy || !unitId || !selectedFile} type="submit">{selectedFile ? `Validar ${selectedFile.name}` : 'Selecione um arquivo'}</button></form>}
    </div>
    {awaitingApproval && <section className="operations-preview" aria-label="Prévia da importação"><header><div><span className="operations-kicker">Prévia antes de efetivar</span><h3>{awaitingApproval.originalFileName ?? 'Importação validada'}</h3><p>Nenhum dado foi gravado na UBS até esta etapa.</p></div><span className="job-status job-status--awaitingapproval">Aguardando aprovação</span></header><div><article><small>Imóveis a criar</small><strong>{awaitingApproval.recordCount.toLocaleString('pt-BR')}</strong></article><article><small>Famílias a criar</small><strong>{awaitingApproval.familyCount ?? 0}</strong></article><article><small>Vínculos a criar</small><strong>{awaitingApproval.linkCount ?? 0}</strong></article><article><small>Erros de validação</small><strong>{awaitingApproval.validationErrorCount.toLocaleString('pt-BR')}</strong></article><article><small>Progresso do fluxo</small><strong>{awaitingApproval.progressPercentage}%</strong></article></div><button className="operations-primary" disabled={busy} onClick={() => void action(awaitingApproval, 'approve')} type="button">Aprovar {awaitingApproval.recordCount.toLocaleString('pt-BR')} imóveis</button></section>}
...
function OperationMetric({ detail, label, value }: { detail: string; label: string; value: number | undefined }) { return <article><small>{label}</small><strong>{value ?? '—'}</strong><p>{detail}</p></article> }
