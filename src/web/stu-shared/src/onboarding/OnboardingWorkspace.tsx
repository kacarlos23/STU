...
export function OnboardingWorkspace({ session, global = false }: { session: Session; global?: boolean }) {
  ...
  return <section className="onboarding-workspace">
    ...
    {loading && !report ? <p className="onboarding-loading" role="status">Verificando a UBS…</p> : report && <>
      ...
      <div className="onboarding-layout"><section className="onboarding-checks"><header><div><span>Validação automática</span><h3>Itens obrigatórios</h3></div><strong>{report.checks.filter(item => item.status === 'passed').length}/{report.checks.length}</strong></header>{report.checks.map(item => <article className={item.status === 'passed' ? 'check-passed' : 'check-blocked'} key={item.id}><i aria-hidden="true">{item.status === 'passed' ? '✓' : '!'}</i><div><strong>{item.label}</strong><p>{item.detail}</p></div><span>{item.status === 'passed' ? 'Aprovado' : 'Bloqueado'}</span></article>)}</section>
