...
public sealed class OperationJobProcessor(StuDbContext db, IConfiguration configuration, ILogger<OperationJobProcessor> logger)
{
    ...
    public async Task<bool> ProcessNextAsync(CancellationToken cancellationToken)
    {
        ...
        {
            LogJobFailure(logger, job.Id, null);
    ...
    private async Task ExportAsync(OperationJob job, CancellationToken cancellationToken)
    {
        ...
        var familyLinks = await db.FamilyPropertyLinks.AsNoTracking().Where(l => ids.Contains(l.PropertyId) && l.EndedAtUtc == null).ToDictionaryAsync(l => l.PropertyId, cancellationToken);
        var familyIds = familyLinks.Values.Select(l => l.FamilyId).ToArray();
        var families = await db.Families.AsNoTracking().Where(f => familyIds.Contains(f.Id)).ToDictionaryAsync(f => f.Id, cancellationToken);
        var visits = await db.PropertyVisits.AsNoTracking().Where(item => familyIds.Contains(item.FamilyId) && item.ArchivedAtUtc == null && (!filters.FromUtc.HasValue || item.VisitedAtUtc >= filters.FromUtc) && (!filters.ToUtc.HasValue || item.VisitedAtUtc <= filters.ToUtc)).ToListAsync(cancellationToken);
        var lastVisits = visits.GroupBy(item => item.FamilyId).ToDictionary(group => group.Key, group => group.OrderByDescending(item => item.VisitedAtUtc).First());
        ...
        var records = properties.Select(item => new ExportProperty(item, microregions.GetValueOrDefault(item.MicroregionId)?.Code ?? string.Empty, familyLinks.TryGetValue(item.Id, out var link) ? lastVisits.GetValueOrDefault(link.FamilyId) : null, familyLinks.TryGetValue(item.Id, out var current) ? families.GetValueOrDefault(current.FamilyId) : null, familyLinks.GetValueOrDefault(item.Id))).ToList();
    ...
    private async Task ValidateImportAsync(OperationJob job, CancellationToken cancellationToken)
    {
        ...
        for (var index = 0; index < rows.Count; index++)
        {
            ...
            if (string.IsNullOrWhiteSpace(row.Street) || string.IsNullOrWhiteSpace(row.HouseNumber)) { errors.Add($"Linha {line}: logradouro e número do imóvel são obrigatórios."); continue; }
            var responsible = row.FamilyResponsibleName.Trim();
            if ((family.Length == 0) != (responsible.Length == 0)) { errors.Add($"Linha {line}: preencha número e responsável juntos, ou deixe ambos vazios."); continue; }
            if (family.Length > 32 || family.Any(char.IsControl) || responsible.Length > 120 || responsible.Any(char.IsControl) || row.Street.Trim().Length > 180 || row.HouseNumber.Trim().Length > 32 || row.PostalCode?.Length > 16 || row.Complement?.Length > 120) { errors.Add($"Linha {line}: um campo excede o limite ou contém conteúdo inválido."); continue; }
            if (family.Length > 0 && (!seen.Add(family) || existingFamilies.Contains(family))) { errors.Add($"Linha {line}: número familiar já utilizado. Faça o vínculo pelo cadastro de famílias."); continue; }
            ...
            if (!Enum.TryParse<PropertyRegistrationStatus>(row.RegistrationStatus, true, out var status) || !Enum.TryParse<PropertySituation>(row.Situation, true, out var situation) || !Enum.IsDefined(status) || !Enum.IsDefined(situation)) { errors.Add($"Linha {line}: situação inválida."); continue; }
            if (family.Length > 0 && status != PropertyRegistrationStatus.Active) { errors.Add($"Linha {line}: o vínculo exige imóvel ativo."); continue; }
            staged.Add(new(micro.Id, row.Street.Trim(), row.HouseNumber.Trim(), family, responsible, Clean(row.PostalCode), Clean(row.Complement), row.Geometry, status, situation));
        ...
        var familyCount = staged.Count(r => r.FamilyNumber.Length > 0);
        job.AwaitApproval(stagedName, staged.Count, familyCount, familyCount); db.UserNotifications.Add(UserNotification.Create(job.CreatedByUserId, job.HealthUnitId, NotificationKind.Information, "Importação validada", $"{staged.Count} imóvel(is), {familyCount} família(s) e {familyCount} vínculo(s) aguardam sua aprovação.", "/operations"));
    ...
    private async Task CommitImportAsync(OperationJob job, CancellationToken cancellationToken)
    {
        ...
        var families = staged.Where(item => item.FamilyNumber.Length > 0).Select(item => item.FamilyNumber).ToArray();
        if (await db.Families.AnyAsync(item => item.HealthUnitId == job.HealthUnitId && families.Contains(item.Number), cancellationToken)) throw new InvalidDataException("Um número de família passou a ser utilizado após a validação. Envie o arquivo novamente.");
        ...
        foreach (var row in staged)
        {
            ...
            var property = HealthProperty.Create(job.HealthUnitId, row.MicroregionId, row.Street, row.HouseNumber, row.PostalCode, row.Complement, row.Geometry, row.RegistrationStatus, row.Situation);
            if (!string.IsNullOrWhiteSpace(row.FamilyNumber))
            {
                var actorId = job.ApprovedByUserId ?? job.CreatedByUserId;
                var family = Family.Create(job.HealthUnitId, row.FamilyNumber, row.FamilyResponsibleName, actorId);
                db.Families.Add(family); db.FamilyVersions.Add(FamilyVersion.Capture(family, 1, "Import", actorId));
                db.FamilyPropertyLinks.Add(FamilyPropertyLink.Create(job.HealthUnitId, family.Id, property.Id, actorId));
            }
    ...
    private static async Task<List<ImportRow>> ReadGeoJsonAsync(string path, CancellationToken cancellationToken)
    {
        ...
        var rows = new List<ImportRow>(); foreach (var feature in features.EnumerateArray())
        {
            ...
            rows.Add(new(Get("microregionCode"), Get("street"), Get("houseNumber"), Get("familyNumber"), Get("familyResponsibleName"), Get("postalCode"), Get("complement"), geometry, Get("registrationStatus", "Active"), Get("situation", "Occupied")));
        }
        return rows;
    }
    ...
    {
        var builder = new StringBuilder("propertyId,microregionCode,street,houseNumber,familyNumber,familyResponsibleName,familyId,linkId,linkStartedAtUtc,postalCode,complement,situation,registrationStatus,lastVisitAtUtc,lastVisitOutcome,lastVisitPropertyId,longitude,latitude\r\n");
        foreach (var row in records) { var point = row.Property.Geometry.Centroid; builder.AppendLine(string.Join(',', row.Property.Id, Csv(row.MicroregionCode), Csv(row.Property.Street), Csv(row.Property.HouseNumber), Csv(row.Family?.Number), Csv(row.Family?.ResponsibleName), row.Family?.Id.ToString() ?? "", row.Link?.Id.ToString() ?? "", row.Link?.StartedAtUtc.ToString("O") ?? "", Csv(row.Property.PostalCode), Csv(row.Property.Complement), row.Property.Situation, row.Property.RegistrationStatus, row.LastVisit?.VisitedAtUtc.ToString("O") ?? "", row.LastVisit?.Outcome.ToString() ?? "", row.LastVisit?.PropertyId.ToString() ?? "", point.X.ToString(CultureInfo.InvariantCulture), point.Y.ToString(CultureInfo.InvariantCulture))); }
        return builder.ToString();
    }

    private static string BuildGeoJson(List<ExportProperty> records) => JsonSerializer.Serialize(new { type = "FeatureCollection", features = records.Select(row => new { type = "Feature", id = row.Property.Id, geometry = row.Property.Geometry, properties = new { propertyId = row.Property.Id, row.MicroregionCode, row.Property.Street, row.Property.HouseNumber, familyNumber = row.Family?.Number, familyResponsibleName = row.Family?.ResponsibleName, familyId = row.Family?.Id, linkId = row.Link?.Id, linkStartedAtUtc = row.Link?.StartedAtUtc, situation = row.Property.Situation.ToString(), registrationStatus = row.Property.RegistrationStatus.ToString(), lastVisitAtUtc = row.LastVisit?.VisitedAtUtc, lastVisitPropertyId = row.LastVisit?.PropertyId } }) }, GeoJsonOptions);
    private static string BuildKml(List<ExportProperty> records) { var body = string.Join("", records.Select(row => $"<Placemark><name>{SecurityElement.Escape(row.Property.HouseNumber)} · F {SecurityElement.Escape(row.Family?.Number ?? "Sem família")}</name><description>{SecurityElement.Escape(row.Property.Street)} · {SecurityElement.Escape(row.Family?.ResponsibleName)}</description><ExtendedData><Data name=\"propertyId\"><value>{row.Property.Id}</value></Data><Data name=\"familyId\"><value>{row.Family?.Id}</value></Data><Data name=\"linkId\"><value>{row.Link?.Id}</value></Data><Data name=\"linkStartedAtUtc\"><value>{row.Link?.StartedAtUtc:O}</value></Data></ExtendedData>{KmlGeometry(row.Property.Geometry)}</Placemark>")); return $"<?xml version=\"1.0\" encoding=\"UTF-8\"?><kml xmlns=\"http://www.opengis.net/kml/2.2\"><Document>{body}</Document></kml>"; }
    ...
    private sealed record ExportProperty(HealthProperty Property, string MicroregionCode, PropertyVisit? LastVisit, Family? Family, FamilyPropertyLink? Link);
    private sealed record ImportRow(string MicroregionCode, string Street, string HouseNumber, string FamilyNumber, string FamilyResponsibleName, string? PostalCode, string? Complement, Geometry Geometry, string RegistrationStatus, string Situation);
    private sealed record StagedProperty(Guid MicroregionId, string Street, string HouseNumber, string FamilyNumber, string FamilyResponsibleName, string? PostalCode, string? Complement, Geometry Geometry, PropertyRegistrationStatus RegistrationStatus, PropertySituation Situation);
