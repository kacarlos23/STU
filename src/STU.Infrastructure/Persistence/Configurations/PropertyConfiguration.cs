...
internal sealed class PropertyConfiguration : IEntityTypeConfiguration<HealthProperty>
{
    public void Configure(EntityTypeBuilder<HealthProperty> b)
    {
        ...
        });b.HasKey(x=>x.Id);b.HasAlternateKey(x=>new{x.Id,x.HealthUnitId});
        b.Property(x=>x.Street).HasMaxLength(180).IsRequired();b.Property(x=>x.HouseNumber).HasMaxLength(32).IsRequired();
        ...
        b.HasIndex(x=>x.MicroregionId);b.HasIndex(x=>x.Geometry).HasMethod("gist");
...
{
    public void Configure(EntityTypeBuilder<PropertyVersion>b){b.ToTable("property_versions");b.HasKey(x=>x.Id);b.Property(x=>x.Street).HasMaxLength(180);b.Property(x=>x.HouseNumber).HasMaxLength(32);b.Property(x=>x.PostalCode).HasMaxLength(16);b.Property(x=>x.Complement).HasMaxLength(120);b.Property(x=>x.Geometry).HasColumnType("geometry(Geometry,4326)");b.Property(x=>x.RegistrationStatus).HasConversion<string>().HasMaxLength(24);b.Property(x=>x.Situation).HasConversion<string>().HasMaxLength(24);b.Property(x=>x.ChangeKind).HasMaxLength(32);b.HasIndex(x=>new{x.PropertyId,x.VersionNumber}).IsUnique();b.HasIndex(x=>new{x.HealthUnitId,x.ChangedAtUtc});}
}

internal sealed class PropertyVisitConfiguration:IEntityTypeConfiguration<PropertyVisit>
{
    public void Configure(EntityTypeBuilder<PropertyVisit>b){b.ToTable("property_visits");b.HasKey(x=>x.Id);b.Property(x=>x.Type).HasConversion<string>().HasMaxLength(24);b.Property(x=>x.Outcome).HasConversion<string>().HasMaxLength(24);b.Property(x=>x.ObservedSituation).HasConversion<string>().HasMaxLength(24);b.Property(x=>x.Note).HasMaxLength(240);b.Property(x=>x.ConcurrencyToken).IsConcurrencyToken();b.HasIndex(x=>new{x.FamilyId,x.VisitedAtUtc});b.HasIndex(x=>new{x.PropertyId,x.VisitedAtUtc});b.HasIndex(x=>new{x.HealthUnitId,x.VisitedAtUtc});b.HasOne<STU.Domain.Families.Family>().WithMany().HasForeignKey(x=>new{x.FamilyId,x.HealthUnitId}).HasPrincipalKey(x=>new{x.Id,x.HealthUnitId}).OnDelete(DeleteBehavior.Restrict);b.HasOne<HealthProperty>().WithMany().HasForeignKey(x=>new{x.PropertyId,x.HealthUnitId}).HasPrincipalKey(x=>new{x.Id,x.HealthUnitId}).OnDelete(DeleteBehavior.Restrict);b.HasOne<STU.Infrastructure.Identity.ApplicationUser>().WithMany().HasForeignKey(x=>x.AgentId).OnDelete(DeleteBehavior.Restrict);}
}
